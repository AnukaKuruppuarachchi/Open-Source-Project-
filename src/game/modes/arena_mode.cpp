...
void arena_mode::setup_round(
	...
	*/
	...
	if (state != arena_mode_state::WARMUP) {
		const auto is_ctf = std::holds_alternative<capture_the_flag_rules>(in.rules.subrules);

		if (in.rules.has_bomb_mechanics() && !is_ctf) {
		...
		if (const auto ctf_rules = std::get_if<capture_the_flag_rules>(&in.rules.subrules)) {
			/* Remove any pre-existing flag entities (from map or previous round) before spawning fresh ones */
			{
				deletion_queue q;
				cosm.for_each_having<components::item>([&](const auto& handle) {
					if (entity_flavour_id(handle.get_flavour_id()) == entity_flavour_id(ctf_rules->flag_flavour)) {
						q.push_back(entity_id(handle.get_id()));
					}
				});
				::reverse_perform_deletions(q, cosm);
			}

			const auto ctf_participating = calc_participating_factions(in);

			ctf_participating.for_each([&](const faction_type flag_owner) {
...
void arena_mode::process_win_conditions(const input_type in, const logic_step step) {
	...
	const auto is_ctf_mode = std::holds_alternative<capture_the_flag_rules>(in.rules.subrules);

	if (in.rules.has_bomb_mechanics() && !is_ctf_mode) {
...
void arena_mode::handle_special_commands(const input_type in, const mode_entropy& entropy, const logic_step step) {
	...
	auto handle_game_command = [&]<typename G>(const G& cmd) {
		if constexpr(std::is_same_v<G, no_arg_game_command>) {
			...
			if (!std::holds_alternative<capture_the_flag_rules>(in.rules.subrules)) {
...
void arena_mode::mode_pre_solve(const input_type in, const mode_entropy& entropy, const logic_step step) {
	...
	if (state == arena_mode_state::WARMUP) {
		...
		if (get_round_end_seconds_left(in) <= 0.f) {
			...
			const auto rst = std::holds_alternative<capture_the_flag_rules>(in.rules.subrules)
				? round_start_type::DONT_KEEP_EQUIPMENTS
				: round_start_type::KEEP_EQUIPMENTS
			;

			start_next_round(in, step, rst);
		}
	}
...
void arena_mode::mode_post_solve(const input_type in, const mode_entropy& entropy, const logic_step step) {
	...
	if (state == arena_mode_state::LIVE) {
		...
		if (const auto ctf_rules = std::get_if<capture_the_flag_rules>(&in.rules.subrules)) {
			...
			const auto ctf_factions = calc_participating_factions(in);

			ctf_factions.for_each([&](const faction_type flag_owner) {
